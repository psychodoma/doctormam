...
$skin_file = $skinroot[0].$skinroot[1].'/list.skin.php';

$category_option = '';
/*if ($qaconfig['qa_category']) {
    $category_href = G5_ADMIN_URL.'/dr_qalist.php';
...
if(is_file($skin_file)) {
    ...
    if($member["mb_level"] < 9)
        $sql_search .= " and branch = '".$member["mb_no"]."' ";
    ...
    for($i=0; $row=sql_fetch_array($result); $i++) {
        ...
        $list[$i]['view_href'] = G5_ADMIN_URL.'/dr_qaview.php?qa_id='.$row['qa_id']."&".$qstr;
    ...
    $list_href = G5_ADMIN_URL.'/dr_qalist.php';
    $write_href = G5_ADMIN_URL.'/dr_qawrite.php';

    $list_pages = preg_replace('/(\.php)(&amp;|&)/i', '$1?', get_paging(G5_IS_MOBILE ? $qaconfig['qa_mobile_page_rows'] : $qaconfig['qa_page_rows'], $page, $total_page, './dr_qalist.php?'.$qstr.'&amp;page='));
