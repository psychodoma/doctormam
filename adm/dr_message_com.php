...
sql_fetch(   "update g5_write_reservation set wr_2 = date_format(now(),'%Y-%m-%d %H:%i:%s') where wr_id = ".$tmp_array[$i] );
   // $val = sql_fetch(   "select pro_type,wr_name,phone,res_branch from g5_write_promotion where wr_id = ".$tmp_array[$i] );
 ...
 //   sql_query(   "update g5_write_promotion set pro_send_date = NOW() where wr_id = ".$tmp_array[$i]    );
}
...
//goto_url('./dr_board.php?bo_table=promotion&pro_option=all');
...
// goto_url('./dr_board.php?bo_table='.$bo_table.'&amp;page='.$page.$qstr);
?>
