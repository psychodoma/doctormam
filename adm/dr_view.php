...
if($bo_table == "review")
{
	$board_skin_path = G5_ADMIN_PATH.'/skin/board/review';
}
if($bo_table == "freegift")
{
	$board_skin_path = G5_ADMIN_PATH.'/skin/board/freegift';
}
...
if ($sca || $stx) {
    ...
    $search_href = './dr_board.php?bo_table='.$bo_table.'&amp;page='.$page.$qstr;
    $list_href = './dr_board.php?bo_table='.$bo_table;
} else {
    $search_href = '';
    $list_href = './dr_board.php?bo_table='.$bo_table.'&amp;page='.$page;
}
...
if (isset($prev['wr_id']) && $prev['wr_id']) {
    ...
    $prev_href = './dr_board.php?bo_table='.$bo_table.'&amp;wr_id='.$prev['wr_id'].$qstr;
}
...
if (isset($next['wr_id']) && $next['wr_id']) {
    ...
    $next_href = './dr_board.php?bo_table='.$bo_table.'&amp;wr_id='.$next['wr_id'].$qstr;
}
...
if ($member['mb_level'] >= $board['bo_reply_level'])
    $reply_href = './dr_write.php?w=r&amp;bo_table='.$bo_table.'&amp;wr_id='.$wr_id.$qstr;

// 수정, 삭제 링크
$update_href = $delete_href = '';
// 로그인중이고 자신의 글이라면 또는 관리자라면 비밀번호를 묻지 않고 바로 수정, 삭제 가능
if (($member['mb_id'] && ($member['mb_id'] == $write['mb_id'])) || $is_admin) {
    $update_href = './dr_write.php?w=u&amp;bo_table='.$bo_table.'&amp;wr_id='.$wr_id.'&amp;page='.$page.$qstr;
    set_session('ss_delete_token', $token = uniqid(time()));
    $delete_href ='./dr_delete.php?bo_table='.$bo_table.'&amp;wr_id='.$wr_id.'&amp;token='.$token.'&amp;page='.$page.urldecode($qstr);
}
else if (!$write['mb_id']) { // 회원이 쓴 글이 아니라면
    $update_href = './dr_password.php?w=u&amp;bo_table='.$bo_table.'&amp;wr_id='.$wr_id.'&amp;page='.$page.$qstr;
    $delete_href = './dr_password.php?w=d&amp;bo_table='.$bo_table.'&amp;wr_id='.$wr_id.'&amp;page='.$page.$qstr;
}

// 최고, 그룹관리자라면 글 복사, 이동 가능
$copy_href = $move_href = '';
if ($write['wr_reply'] == '' && ($is_admin == 'super' || $is_admin == 'group')) {
    $copy_href = './dr_move.php?sw=copy&amp;bo_table='.$bo_table.'&amp;wr_id='.$wr_id.'&amp;page='.$page.$qstr;
    $move_href = './dr_move.php?sw=move&amp;bo_table='.$bo_table.'&amp;wr_id='.$wr_id.'&amp;page='.$page.$qstr;
}
