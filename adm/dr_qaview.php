...
if(strpos($qa_skin_path, "mobile")){
	//$qa_skin_path = "../skin/qa/";
	$skinroot = explode('mobile/',$qa_skin_path);
	$skin_file = $skinroot[0].$skinroot[1];

	$qa_skin_path =  $skin_file;
}
...
if(is_file($skin_file)) {
    ...
    $update_href = '';
    $delete_href = '';
    $write_href = G5_ADMIN_URL.'/dr_qawrite.php';
    $rewrite_href = G5_ADMIN_URL.'/dr_qawrite.php?w=r&amp;qa_id='.$view['qa_id'];
    $list_href = G5_ADMIN_URL.'/dr_qalist.php?'.preg_replace('/^&amp;/', '?', $qstr);
    ...
    */
    if(($view['qa_type'] && $is_admin) || (!$view['qa_type'] && $view['qa_status'] == 0)) {
        $update_href = G5_ADMIN_URL.'/dr_qawrite.php?w=u&amp;qa_id='.$view['qa_id']."&".$qstr;
        $delete_href = G5_ADMIN_URL.'/dr_qadelete.php?qa_id='.$view['qa_id']."&".$qstr;
    }
    ...
    if(!$view['qa_type'] && $view['qa_status']) {
        ...
        if($is_admin || $member["mb_level"] > 6) {
            $answer_update_href = G5_ADMIN_URL.'/dr_qawrite.php?w=u&amp;qa_id='.$answer['qa_id']."&".$qstr;
            $answer_delete_href = G5_ADMIN_URL.'/dr_qadelete.php?qa_id='.$answer['qa_id']."&".$qstr;
        }
    }
