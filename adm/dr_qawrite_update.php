...
if($w == 'u' || $w == 'a' || $w == 'r') {
    if($w == 'a' && !$is_admin && !$member["mb_level"] > 6)
...
if($config['cf_sms_use'] == 'icode' && $qaconfig['qa_use_sms']) {
		...
		//echo "SMS | ".$write['qa_sms_recv']." / ". trim($write['qa_hp']);
    ...
    if($config['cf_sms_type'] == 'LMS') {
        ...
        if($w == 'a' && $write['qa_sms_recv'] && trim($write['qa_hp'])) {
			...
			if($_SERVER["REMOTE_ADDR"] == "210.96.212.116" ){
				echo $sms_content." / ".$send_number." / ".$recv_number;
			}
...
if($w == 'a')
    $result_url = G5_ADMIN_URL.'/dr_qaview.php?qa_id='.$qa_id.$qstr;
else if($w == 'u' && $write['qa_type'])
    $result_url = G5_ADMIN_URL.'/dr_qaview.php?qa_id='.$write['qa_parent'].$qstr;
else
    $result_url = G5_ADMIN_URL.'/dr_qalist.php?'.preg_replace('/^&amp;/', '?', $qstr);
