...
if ($sca || $stx || $_REQUEST["branch"]) {
        if(empty($stx) && $_REQUEST["branch"]){
        $sql_search = " 1=1 ";
    }else{
        $sql_search = get_sql_search($sca, $sfl, $stx, $sop);

		//카테고리가 있고, 보드테이블이 닥터맘교육이고, 검색어 값이 없을때 시작 2017-01-20 lkhee
		if($sca && $bo_table == 'edu' && empty($stx)) { 
		...
		//카테고리가 있고, 보드테이블이 닥터맘교육이고, 검색어 값이 없을때 종료 2017-01-20 lkhee
    ...
    if(!empty($_REQUEST["branch"])){
        $sql_search .= " and (branch = '{$_REQUEST["branch"]}' or branch = '".get_branch($_REQUEST["branch"],"mb_nick") ."' )";
    }
...
if($bo_table=="reservation"){
	if($sql_search){
		$sql = " SELECT COUNT(DISTINCT `wr_parent`) AS `cnt` FROM {$write_table} WHERE {$sql_search} {$sql_order} ";
	}else{
...
// 공지 처리
// !$sca && 값이 없을때 조건 제거 2017-01-20 lkhee
// && !$_REQUEST["branch"] 이용후기에 카테고리로만 검색이 있어서 추가 2017-01-20 lkhee
if (!$stx  && !$_REQUEST["branch"]) {
    $arr_notice = explode(',', trim($board['bo_notice']));
	...
	// 닥터맘교육일경우 시작 2017-01-20 lkhee
	if($sca && $bo_table == 'edu') {
		$ca_name_search = "and ca_name = '{$sca}'";
	}
	// 닥터맘교육일경우 종료 2017-01-20 lkhee

    for ($k=0; $k<$board_notice_count; $k++) {
        if (trim($arr_notice[$k]) == '') continue;

        $row = sql_fetch(" select * from {$write_table} where wr_id = '{$arr_notice[$k]}' {$ca_name_search} ");
...
// 관리자라면 CheckBox 보임
$is_checkbox = false;
if ($is_member && ($is_admin == 'super' || $group['gr_admin'] == $member['mb_id'] || $board['bo_admin'] == $member['mb_id'] ||  6 < $member['mb_level']) )
    $is_checkbox = true;

// 정렬에 사용하는 QUERY_STRING
$qstr2 = 'bo_table='.$bo_table.'&amp;sop='.$sop.'&branch='.$branch;
...
if (!$sst) {
    if ($board['bo_sort_field']) {
		...
		$sst  = "wr_num, wr_reply asc, wr_datetime";
        $sod = "";
    }
} else {
    ...
    $sst = preg_match("/^(wr_datetime|wr_name|phone|service_start|wr_hit|wr_good|wr_nogood)$/i", $sst) ? $sst : "";
}
...
$write_pages = get_paging(G5_IS_MOBILE ? $config['cf_mobile_pages'] : $config['cf_write_pages'], $page, $total_page, './dr_board.php?bo_table='.$bo_table.'&'.$qstr2.'&amp;page=');
