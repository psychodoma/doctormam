...
for ($i=$chk_count-1; $i>=0; $i--)
{
	...
	else if ($member['mb_level'] == 7) // 지사인 경우(지사레벨 :7) 2017-01-25 lkhee
	{
		;
	}
    else
    ...
    if($write_table == 'g5_write_reservation'){
        //echo $write['wr_id'];
        //exit();
        sql_query(" delete from g5_write_promotion where pro_id = '{$write['wr_id']}' ");
    }
...
goto_url('./dr_board.php?bo_table='.$bo_table.'&pro_option=all&amp;page='.$page.$qstr);
?>
