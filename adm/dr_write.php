...
if (!$bo_table) {
    alert("bo_table 값이 넘어오지 않았습니다.\\ndr_write.php?bo_table=code 와 같은 방식으로 넘겨 주세요.", G5_URL);
}
...
if ($w == '') {
    if ($wr_id) {
        alert('글쓰기에는 \$wr_id 값을 사용하지 않습니다.', G5_ADMIN_URL.'/dr_board.php?bo_table='.$bo_table);
    }
...
$action_url = https_url(G5_ADMIN_DIR)."/dr_write_update.php";
...
if($bo_table == "app")
{
	$board_skin_path = G5_ADMIN_PATH.'/skin/board/app';
}
if($bo_table == "promotion")
{
    $board_skin_path = G5_ADMIN_PATH.'/skin/board/promotion';
}
