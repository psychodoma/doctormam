...
function get_paging2($write_pages, $cur_page, $total_page, $url, $add="")
{
    ...
    $str = '';
    if ($cur_page > 1) {
        $str .= '<li class="pprev"><a href="'.$url.'1'.$add.'" ><img src="'.G5_IMG_URL.'/sub/con_pprev.jpg"></a><li>'.PHP_EOL;
    }
    ...
    if ($start_page > 1) $str .= '<li class="prev"><a href="'.$url.($start_page-1).$add.'" ><img src="'.G5_IMG_URL.'/sub/con_prev.jpg"></a></li>'.PHP_EOL;

    if ($total_page > 1) {
        for ($k=$start_page;$k<=$end_page;$k++) {
            if ($cur_page != $k)
                $str .= '<li><a href="'.$url.$k.$add.'" >'.$k.'<span class="sound_only">페이지</span></a></li>'.PHP_EOL;
            else
                $str .= '<li class="board_strong">'.$k.'</li>'.PHP_EOL;
        }
    }

    if ($total_page > $end_page) $str .= '<li class="next"><a href="'.$url.($end_page+1).$add.'" ><img src="'.G5_IMG_URL.'/sub/con_next.jpg"></a></li>'.PHP_EOL;

    if ($cur_page < $total_page) {
        $str .= '<li class="nnext"><a href="'.$url.$total_page.$add.'" ><img src="'.G5_IMG_URL.'/sub/con_nnext.jpg"></a></li>'.PHP_EOL;
    }

    if ($str)
        return "<div class=\"board_ft\"><ul class=\"board_list\">{$str}</ul></div>";
    else
        return "";
}
...
{
    //$url = preg_replace('#&amp;page=[0-9]*(&amp;page=)$#', '$1', $url);
    $url = preg_replace('#&amp;currentPage=[0-9]*#', '', $url) . '&amp;currentPage=';
...
function get_list($write_row, $board, $skin_url, $subject_len=40, $adm_board=null)
{
    ...
    $list['ca_name_href'] = G5_BBS_URL.'/'.$adm_board.'board.php?bo_table='.$board['bo_table'].'&amp;sca='.urlencode($list['ca_name']);
    if($adm_board){
    $list['href'] = G5_ADMIN_URL.'/'.$adm_board.'board.php?bo_table='.$board['bo_table'].'&amp;wr_id='.$list['wr_id'].$qstr;
    }else{
    $list['href'] = G5_BBS_URL.'/'.$adm_board.'board.php?bo_table='.$board['bo_table'].'&amp;wr_id='.$list['wr_id'].$qstr;
    }
...
function get_member_name_by_no($mb_no)
{
    global $g5;

    $result= sql_fetch(" select mb_nick from {$g5['member_table']} where mb_no = TRIM('$mb_no') ");
...
function radio_selected($name, $value, $selected, $text='')
{
    if (!$text) $text = $value;
    if ($value == $selected)
        return "<input type=\"radio\" name=\"$name\" id=\"".$name."_".$value."\" value=\"$value\" checked ><label for=\"".$name."_".$value."\">$text</label>\n";
    else
        return "<input type=\"radio\" name=\"$name\" id=\"".$name."_".$value."\" value=\"$value\" ><label for=\"".$name."_".$value."\">$text</label>\n";
}
...
// 지사를 SELECT 형식으로 얻음
function get_branch_name_select($name, $selected="", $event="",$nothing="")
{
    global $g5, $member;
    if($member["mb_level"]<9){
        $add_where = " and mb_no = ". $member["mb_no"];
    }
    $sql = " select mb_branch,mb_no from {$g5['member_table']} where (mb_level = '7' or mb_level = '9') and (".get_location('서울').")".$add_where  ." order by mb_level desc, mb_nick asc";
    $result = sql_query($sql);
    $str = '<select id="'.$name.'" name="'.$name.'" '.$event.'>';
    if($nothing = "y"){
    $str .= "<option value=''>지사선택</option>";
    $str .= '<optgroup label="서울">';
    }
    for ($i=0; $row=sql_fetch_array($result); $i++)
    {
        $str .= '<option value="'.$row['mb_no'].'"';
        if ($row['mb_no'] == $selected) $str .= ' selected';
        $str .= '>'.$row['mb_branch'].'</option>';
    }

    $sql = " select mb_branch,mb_no from {$g5['member_table']} where (mb_level = '7' or mb_level = '9') and (".get_location('경인').")".$add_where  ." order by mb_level desc, mb_nick asc";
    $result = sql_query($sql);
    if($nothing = "y"){
    $str .= '<optgroup label="경인">';
    }
    for ($i=0; $row=sql_fetch_array($result); $i++)
    {
        $str .= '<option value="'.$row['mb_no'].'"';
    ...
    $sql = " select mb_branch,mb_no from {$g5['member_table']} where (mb_level = '7' or mb_level = '9') and (".get_location('경기').")".$add_where  ." order by mb_level desc, mb_nick asc";
    $result = sql_query($sql);
    if($nothing = "y"){
    $str .= '<optgroup label="경기">';
    }
    for ($i=0; $row=sql_fetch_array($result); $i++)
    {
        $str .= '<option value="'.$row['mb_no'].'"';
    ...
    $sql = " select mb_branch,mb_no from {$g5['member_table']} where (mb_level = '7' or mb_level = '9') and (".get_location('충청/전북').")".$add_where  ." order by mb_level desc, mb_nick asc";
    $result = sql_query($sql);
    if($nothing = "y"){
    $str .= '<optgroup label="충청/전북">';
    }
    for ($i=0; $row=sql_fetch_array($result); $i++)
    {
        $str .= '<option value="'.$row['mb_no'].'"';
    ...
    $sql = " select mb_branch,mb_no from {$g5['member_table']} where (mb_level = '7' or mb_level = '9') and (".get_location('경상남북').")".$add_where  ." order by mb_level desc, mb_nick asc";
    $result = sql_query($sql);
    if($nothing = "y"){
    $str .= '<optgroup label="경상남북">';
    }
    for ($i=0; $row=sql_fetch_array($result); $i++)
    {
        $str .= '<option value="'.$row['mb_no'].'"';
...
function get_all_branch_select($name, $selected="", $event=""){
    global $g5;

    $str = '<select id="'.$name.'" name="'.$name.'" '.$event.'>';
    $str .= "<option value=''>지사선택</option>";
    $sql = " select mb_branch,mb_no from {$g5['member_table']} where mb_level = '7' and (".get_location('서울') . ")";
    $result = sql_query($sql);
    $str .= '<optgroup label="서울">';
    for ($i=0; $row=sql_fetch_array($result); $i++)
    {
    ...
    $sql = " select mb_branch,mb_no from {$g5['member_table']} where mb_level = '7' and (".get_location('경인').")";
    $result = sql_query($sql);
    $str .= '<optgroup label="경인">';
    for ($i=0; $row=sql_fetch_array($result); $i++)
    {
    ...
    $sql = " select mb_branch,mb_no from {$g5['member_table']} where mb_level = '7' and (".get_location('경기').")";
    $result = sql_query($sql);
    $str .= '<optgroup label="경기">';
    for ($i=0; $row=sql_fetch_array($result); $i++)
    {
    ...
    $sql = " select mb_branch,mb_no from {$g5['member_table']} where mb_level = '7' and (".get_location('충청/전북').")";
    ...
    $sql = " select mb_branch,mb_no from {$g5['member_table']} where mb_level = '7' and (".get_location('경상남북').")";
    $result = sql_query($sql);
    $str .= '<optgroup label="경상남북">';
    for ($i=0; $row=sql_fetch_array($result); $i++)
    {
...
// 지사를 SELECT 형식으로 얻음
function get_manager_name_select($name, $selected="", $event="",$service_start,$service_end)
{
    global $g5, $member;
    if($member["mb_level"]<7){
        $add_where = " and mb_id = ". $member["mb_id"];
    }else if($member["mb_level"] == 7){
        $add_where = " and mb_branch = ". $member["mb_no"];
    }
    $sql = " select mb_name,mb_nick,mb_id,mb_no,mb_hp ,(select mb_nick from g5_member b where a.mb_branch = b.mb_no) branch_group from {$g5['member_table']} a where mb_level = '5' ".$add_where." order by branch_group, mb_nick";
    $result = sql_query($sql);
    $str = '<select id="'.$name.'" name="'.$name.'" '.$event.'>';
    $before_branch = "";
    $k = 0;
    $str .= '<option value="">관리사지정</option>';
    for ($i=0; $row=sql_fetch_array($result); $i++)
    {
        if($row["branch_group"] != $before_branch){
            $str .= '<optgroup label="'.$row["branch_group"].'">';
            $k++;
        }
        $display_none = "";
        if(!empty($service_start)){
            $sql = "select * from g5_write_reservation where manager = '{$row["mb_no"]}' and (service_start <= '{$service_end}' and date_add(service_start,interval +service_period week) >= '{$service_start}') ";
            $rs = sql_fetch($sql);
            if(!empty($rs)){
                $display_none = " style='display:none;'";
            }
        }
        $str .= '<option value="'.$row['mb_id'].'" info="'.$row['mb_hp'].'"';
        ...
        $before_branch = $row["branch_group"];
    }
    $str .= '</select>';
    return $str;
}
// 휴대전화 첫째자리를 SELECT 형식으로 얻음
function get_phone_select($name, $selected="", $event="")
{
    $numbers = array("010","011","016","017","018","019");
    $str = '<select id="'.$name.'" name="'.$name.'" '.$event.'>';
    for ($i=0; $i < count($numbers); $i++)
    {
        $str .= '<option value="'.$numbers[$i].'"';
        if ($numbers[$i] == $selected) $str .= ' selected';
        $str .= '>'.$numbers[$i].'</option>';
    }
    $str .= '</select>';
    return $str;
}
// 전화번호 첫째자리를 SELECT 형식으로 얻음
function get_tel_select($name, $selected="", $event="")
{
    $numbers = array('02','031','032','033','041','042','043','051','052','053','054','055','061','062','063','064',"070");
    $str = '<select id="'.$name.'" name="'.$name.'" '.$event.'>';
    for ($i=0; $i < count($numbers); $i++)
    {
        $str .= '<option value="'.$numbers[$i].'"';
...
{
    $numbers = array("알뜰형","일반형","베스트","프리미엄","명품플러스","바우처");
    if(is_numeric($num)){
       return $numbers[$num];
    }else{
        return $num;
...
function get_service_color($num)
{
    $numbers = array("#FFFF33","#f2bb74","#7d9cf2","#f29c74","#cc74f2","#7cf274");
    if(is_numeric($num)){
       return $numbers[$num];
    }else{
        return "#898989";
...
{
    $numbers = array("","첫째출산예정","둘째출산예정","셋째출산예정","그이상");
    if(is_numeric($num)){
       return $numbers[$num];
    }else{
        return $num;
...
function get_genre($num)
{
    $numbers = array("","입주","출퇴근");
    if(is_numeric($num)){
        return $numbers[$num];
    }else{
        return $num;
...
function get_period($num)
{
    $numbers = array("","1주","2주","3주","4주이상");
    if(is_numeric($num)){
        return $numbers[$num];
    }else{
...
function get_answer5($num)
{
    $numbers = array("","매우불만족","불만족","보통","만족","매우만족");
    if(is_numeric($num)){
        return $numbers[$num];
    }else{
...
function get_answer3($num)
{
    $numbers = array("","그렇지않다","보통","그렇다");
    if(is_numeric($num)){
        return $numbers[$num];
    }else{
...
function get_answers3($num)
{
    $numbers = array("","그렇다","보통","그렇지않다");
    if(is_numeric($num)){
        return $numbers[$num];
    }else{
...
function get_answer12($num,$num_txt)
{
    $numbers = array("",$num_txt,"시/군/구 안내를 통해","주변 친척이나 지인의 소개로","인터넷이나 인쇄물 광고 등을 통해");
    if(is_numeric($num)){
        return $numbers[$num];
    }else{
...
function get_answer2($num)
{
    $numbers = array("","그렇지않다","그렇다");
    if(is_numeric($num)){
        return $numbers[$num];
    }else{
...
function get_workday($num)
{
    $numbers = array("","5일제","6일제");
    if(is_numeric($num)){
        return $numbers[$num];
    }else{
...
{
    $numbers = array("","예약접수","예약확인","예약완료","예약취소");
    if(is_numeric($num)){
        return $numbers[$num];
    }else{
...
// 서비스등급을 SELECT 형식으로 얻음
function get_grade_select($name, $selected="", $event="")
{
    $str = '<select id="'.$name.'" name="'.$name.'" '.$event.'>';
    if($nothing = "y"){
    $str .= "<option value=''>등급선택</option>";
    }
    for ($i=0; $i < 5; $i++)
    {
        if(!empty(get_grade($i)))
        $str .= '<option value="'.get_grade($i).'"';
        if (get_grade($i) == $selected) $str .= ' selected';
        $str .= '>'.get_grade($i).'</option>';
    }
    $str .= '</select>';
    return $str;
}
// 예약상태를 SELECT 형식으로 얻음
function get_status_select($name, $selected="", $event="")
{
    $numbers = array("1","3","4","5");
    $str = '<select id="'.$name.'" name="'.$name.'" '.$event.'>';
    for ($i=0; $i < count($numbers); $i++)
    {
        $str .= '<option value="'.$numbers[$i].'"';
        if ($numbers[$i] == $selected) $str .= ' selected';
        $str .= '>'.StatusToName($numbers[$i]).'</option>';
    }
    $str .= '</select>';
    return $str;
}
...
// 지사 정보를 얻는다
function get_branch($mb_no, $fields='mb_nick')
{
    global $g5;
    $result = sql_fetch(" select $fields from {$g5['member_table']} where mb_no = TRIM('$mb_no') ");
...
function get_freegift_subject($id){   //2017-06-27 김지환   ex) 사은품 id 값을 넣어주면 무슨 사은품인지 알려주는 함수
	$sql = " select * from g5_write_freegift where wr_id = ".$id;
	$result = sql_fetch($sql);
	return $result['wr_subject'];
}

function get_freegift_msg($id){   //2017-06-27 김지환   ex) 사은품 id 값을 넣어주면 무슨 사은품인지 알려주는 함수
	$sql = " select * from g5_write_freegift where wr_id = ".$id;
	$result = sql_fetch($sql);
	return $result['wr_3'];
}

function get_freegift_select(){  //2017-06-27 김지환
	$free_tab_value = sql_query('select * from g5_write_freegift where wr_2 >= current_Date()');
	echo "<select name='wr_1'>";

	echo "<option value='' selected >선택안함</option>";
	while( $row = sql_fetch_array($free_tab_value) ){
		echo "<option value='".$row['wr_id']."' >".$row['wr_subject']."</option>";
	}
	echo $str .= "</select>";
}

function set_freegift_select($id){   //2017-06-27 김지환
	...
	while( $row = sql_fetch_array($free_tab_value) ){
		if( $id == $row['wr_id'] ){
			echo "<option value='".$row['wr_id']."' selected>".$row['wr_subject']."</option>";
		}else{
			echo "<option value='".$row['wr_id']."' >".$row['wr_subject']."</option>";
		}
	}
...
function get_branch_nick($id){  //2017-06-28 김지환     브런치 값을 입력하면 지점 이름으로 출력
	$result = sql_fetch('select * from g5_member where mb_no = '.$id);
	return $result['mb_nick'];
}

function get_selectbox($name,$arr,$val){
	...
	for( $i=0; $i<count($arr); $i++){
		$j = $i+1;
		if($val == $j){
			echo "<option value='".$j."' selected >".$arr[$i]."</option>";
		}else{
			echo "<option value='".$j."' >".$arr[$i]."</option>";
		}
	}
