...
if ($file_upload_msg)
    ...
    if($bo_table == "reservation"){
        $send_list = "";
        if($w == "" ||$w == "r"){
            $sms_content = $wr_name." 고객님 닥터맘 온라인 상담접수가 완료되었습니다. 담당지사에서 \"해피콜\"을 드립니다. 감사합니다.";
            $reply_num  = sql_fetch(" select mb_hp from {$g5['member_table']} where mb_no = TRIM('$wr_branch') ");
            $sms_content2 = $wr_name." 고객님 연락처 ".$wr_hp0." 닥터맘 온라인 예약접수.";
  			send_sms($reply_num["mb_hp"],$reply_num["mb_hp"],$sms_content2); // 2017-04-12 김지환
			send_sms($wr_hp0,$reply_num["mb_hp"],$sms_content); 
        }
        alert("예약이 완료 되었습니다.");
        goto_url(G5_URL."/");
    }else if($bo_table == "app"){
		// 받는번호 , 보내는번호 , 문자내용
		// 받는번호 : 010-9901-3726
		// 보내는 번호 : 닥터맘대표 010-3458-3272
        $sms_content2 = "닥터맘 산후관리사가 지원하였습니다.";
        send_sms('010-9901-3726','010-3458-3272' ,$sms_content2);
		alert("접수 되었습니다.");
        goto_url(G5_URL."/");
    }else{
