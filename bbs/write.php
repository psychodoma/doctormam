...
if($bo_table == "review"){
    if($w == 'u'){ // 수정
		if($is_guest){
			$wr_hp0 = $_REQUEST['wr_hp1'] . "-" . $_REQUEST['wr_hp2'] . "-" . $_REQUEST['wr_hp3'];

			$sql = "select * from g5_write_review where wr_id = '{$_REQUEST['wr_id']}' and wr_1= '{$wr_hp0}'";
			$rs = sql_fetch($sql);
			if (empty($rs)) {
				alert("휴대번호가 틀립니다.", $_SERVER["HTTP_REFERER"]);
			}
		}

	} else if($is_guest) {
        $wr_hp0 = $_REQUEST['wr_hp1'] . "-" . $_REQUEST['wr_hp2'] . "-" . $_REQUEST['wr_hp3'];
        $sql = "select * from g5_member where mb_name = '{$_REQUEST['wr_name']}' and mb_hp= '{$wr_hp0}' and mb_level < 4";
        $rs = sql_fetch($sql);
        if (empty($rs)) {
            alert("일치하는 회원 정보가 없습니다.", $_SERVER["HTTP_REFERER"]);
        }
    }
}
...
$hp = $_POST['wr_hp1'].'-'.$_POST['wr_hp2'].'-'.$_POST['wr_hp3'];
if($_POST['wr_name']){
	$name     = $_POST['wr_name'];
}else{
	$name     = "";
