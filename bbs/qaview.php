...
if($is_guest){
    ...
    $wr_phone0 =  $_REQUEST["wr_hp1"]."-".$_REQUEST["wr_hp2"]."-".$_REQUEST["wr_hp3"];
    $result = sql_fetch("select * from g5_qa_content where qa_name = '{$wr_name}' and qa_hp='{$wr_phone0}' limit 0,1");

    if(empty($result["qa_hp"])){
        alert('본인이 작성한 글이 아니거나 정보가 틀렸습니다.', $_SERVER['HTTP_REFERER']);
