...
$qa_hp = $_REQUEST["qa_hp1"]."-".$_REQUEST["qa_hp2"]."-".$_REQUEST["qa_hp3"];
...
if($is_guest){
    $qa_name = addslashes($_REQUEST["qa_name"]);
}else{
...
$wr_send_list = get_branch($_REQUEST["qa_branch"],"mb_hp");
$wr_reply = get_member("drmamhead","mb_hp");
$wr_message = "문의하기에 ".$qa_name."고객님의 글이 등록되었습니다.";



if($is_guest){
    send_sms($wr_send_list,$wr_reply["mb_hp"],$wr_message);
}
...
if ($file_upload_msg)
    ...
    alert("문의가 접수 되었습니다.빠른 시일내에 답변 드리겠습니다.",$result_url);//goto_url($result_url);
