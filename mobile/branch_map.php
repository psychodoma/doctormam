...
if(!empty($_REQUEST["serach_branch"])){
    $result_branch = sql_fetch("select * from g5_member where mb_addr1 like '%".$_REQUEST["serach_branch"]."%' limit 0,1");
				...
				<div class="tabDetails" style="border:none; margin:0">
					...
					<div id="tab1" class="tabContents1">
						<div class="sub6_t2">
							<ul class="sub6_t2_li">
								 <?php if(!empty($si)) {
									...
									$sql = "select mb_id, mb_branch from g5_member where (mb_level = '7' or mb_level = '9') and ({$add_sql})  order by mb_level desc, mb_nick asc";
									$result = sql_query($sql);
									while ($row = sql_fetch_array($result)) {
										?>
										<li>
											<a href=javascript:SearchLocation("gu","<?php echo $row["mb_id"]; ?>")><?php echo $row["mb_branch"] ?></a>
										</li>
									<?php }
								}?>
							</ul>
						...
						<div>
							...
							<div class="sub6_2_img1"></div>


							<h3 class="sub6_2title" id="information">닥터맘 지사안내 입니다.</h3>
							...
							<div class="sub6_2_tc">
								...
								<table summary="담당지역,전화번호,주소,이메일,계좌안내" class="sub6_2_table">
									...
									<tbody>
										<tr>
											<td class="t_strong3" style="width:19%;">연락처</td>
										...
										<tr>
											<td rowspan="2" class="t_strong3">계좌안내</td>
											<td colspan="3"><span class="strong"><?php echo $location["mb_bank_name"]?> <?php echo $location["mb_bank_account"]?></td>
										</tr>
	<!--								<tr>
											<td colspan="3"><span class="strong">국민 088201-04-166732</span>고현진(닥터맘)</td>
										</tr>-->
									</tbody>
								</table>
							</div>
							<!-- 네이버 지도 -->
							<div id="map" style=" width:100%; height:220px; margin-bottom:30px;"></div>
						<? } else { ?>
							<p style="text-align: center; padding: 30px 0; font-size:14px; letter-spacing:0;">------- 지점을 선택해주세요. -------</p>
						<? } ?>
						</div>
<!-- <script type="text/javascript">
	...
	var oSize = new nhn.api.map.Size(33, 43);
	var oOffset = new nhn.api.map.Size(18, 43);
	var oIcon = new nhn.api.map.Icon(g5_url+'/mobile/img/sub6/sub6_2/map_icon.png', oSize, oOffset);
                    ...
                    <tr>
                      ...
                      <!-- 삼태아/중증장애인 끝 -->
						...
						<div class="tabcont">
							...
							<p class="detail_view_btn"><a href="<?php echo G5_MOBILE_URL ?>/service_best.php">서비스내용 자세히 보기</a></p>
...
<script>
    function SearchLocation(type,str,ids)
    {
        ...
        if(type == "si"){
            ...
            $("#seach_gu").val(str);
            gu_val = str;
            location.href = g5_url+"/mobile/branch_map.php?branch="+gu_val+"&si="+search_val + "#information";
        }
    }
</script>
<?php
