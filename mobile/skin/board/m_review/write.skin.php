...
<link rel="stylesheet" href="<?php echo G5_CSS_URL ?>/jquery.mobile.modal.css">
    ...
    <form name="fwrite" id="fwrite" action="/bbs/write_review_update.php" onsubmit="return fwrite_submit(this);" method="post" enctype="multipart/form-data" autocomplete="off">
	...
	<input type='hidden' name='wr_1' value='<?echo $wr_hp1."-".$wr_hp2."-".$wr_hp3?>'>
    <?php
    $option = '';
    $option_hidden = '';
					...
					<table summary="제목, 이름, 작성일, 조회수, 첨부파일, 지사" class="board_view">
						...
						<tbody>
							...
							<tr>
								...
								<?if($bo_table == "review"){?>
								<td colspan="5"><input type="text" name="wr_subject_ck" value="<?php echo get_text($write['wr_subject_ck']); ?>" id="wr_subject" required class="frm_input"  style="width:95%; padding-left:5px;"></td>
								<?}else{?>
