...
if ($bo_table) {
    ...
    if ($board['bo_table']) {
        ...
        if($bo_table == "reservation"){
            $add_select = ", (select mb_id from g5_member where manager = mb_no) as manager_id, (select mb_id from g5_member where manager2 = mb_no) as manager_id2,date_add(service_start,interval +service_period week) as service_end, (select wr_success from sms5_write where wr_id = wr_fore_key and wr_fore_type = '1' order by wr_datetime desc limit 0,1 ) as sms_1, (select wr_success from sms5_write where wr_id = wr_fore_key and wr_fore_type = '2' order by wr_datetime desc limit 0,1 ) as sms_2, (select wr_success from sms5_write where wr_id = wr_fore_key and wr_fore_type = '3' order by wr_datetime desc limit 0,1 ) as sms_3, (select wr_success from sms5_write where wr_id = wr_fore_key and wr_fore_type = '4' order by wr_datetime desc limit 0,1 ) as sms_4, (select wr_success from sms5_write where wr_id = wr_fore_key and wr_fore_type = '5' order by wr_datetime desc limit 0,1 ) as sms_5, (select wr_success from sms5_write where wr_id = wr_fore_key and wr_fore_type = '6' order by wr_datetime desc limit 0,1 ) as sms_6, (select wr_success from sms5_write where wr_id = wr_fore_key and wr_fore_type = '7' order by wr_datetime desc limit 0,1 ) as sms_7, (select ip_address from question where app_key = wr_id limit 0,1) as question, (select mb_bank_name from g5_member where res_branch = mb_no) as mb_bank_name, (select mb_bank_account from g5_member where res_branch = mb_no) as mb_bank_account";
        }
//echo " select * $add_select from $write_table where wr_id = '$wr_id' ";
        if (isset($wr_id) && $wr_id)
            $write = sql_fetch(" select * $add_select from $write_table where wr_id = '$wr_id' ");
    }
}
...
if($member_type == "본사"){
    $member_auth = "9";
}else if($member_type == "지사"){
    $member_auth = "7";
}else if($member_type == "회원"){
    $member_auth = "5";
}
// 방문자수의 접속을 남김
